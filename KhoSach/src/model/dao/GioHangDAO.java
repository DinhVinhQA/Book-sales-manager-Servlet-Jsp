...
public class GioHangDAO {
    ...
    public ArrayList<GioHangBEAN> getGioHangs(int maKhachHang) {
        ...
        try {
            con.KetNoi();
            String sql = "SELECT GioHang.MaSach, Sach.TenSach, Sach.Anh, Sach.Gia, GioHang.SoLuong FROM     GioHang INNER JOIN Sach ON GioHang.MaSach = Sach.MaSach Where GioHang.MaKhachHang="
                    + maKhachHang + "";
            ...
            while (rs.next()) {
                ...
                GioHangBEAN s = new GioHangBEAN(maSach, tenSach, anh, soLuong,
                        gia);
                ds.add(s);
            }
        } catch (Exception e) {
    ...
    public int capNhapGioHang(int maKhachHang, int maSach, int soLuong) {
        ...
        try {
            con.KetNoi();
            String sql = "UPDATE GioHang SET SoLuong = SoLuong + ? WHERE MaSach=? and MaKhachHang=?";
            PreparedStatement cmd = con.cn.prepareStatement(sql);
